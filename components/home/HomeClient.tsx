...
export default function HomeClient() {
    ...
    const bookingFeatures = [
        { icon: MessageSquare, text: t('booking.features.whatsapp'), color: 'bg-gradient-to-br from-green-500 to-emerald-600', shadow: 'shadow-green-500/30' },
        { icon: Calendar, text: t('booking.features.priority'), color: 'bg-gradient-to-br from-blue-500 to-indigo-600', shadow: 'shadow-blue-500/30' },
        { icon: Clock, text: t('booking.features.rescheduling'), color: 'bg-gradient-to-br from-purple-500 to-violet-600', shadow: 'shadow-purple-500/30' },
        { icon: Shield, text: t('booking.features.payments'), color: 'bg-gradient-to-br from-amber-500 to-orange-600', shadow: 'shadow-amber-500/30' },
    ]

    const serviceHighlights = [
        { icon: Crown, text: t('serviceHighlights.bridalMakeup'), color: 'text-pink-400' },
        { icon: Scissors, text: t('serviceHighlights.hairStyling'), color: 'text-purple-400' },
        { icon: Palette, text: t('serviceHighlights.facials'), color: 'text-blue-400' },
        { icon: Flower2, text: t('serviceHighlights.spaWellness'), color: 'text-green-400' },
        { icon: Gem, text: t('serviceHighlights.nailArt'), color: 'text-amber-400' },
        { icon: Heart, text: t('serviceHighlights.skinCare'), color: 'text-rose-400' },
        { icon: Wand2, text: t('serviceHighlights.threading'), color: 'text-cyan-400' },
        { icon: Star, text: t('serviceHighlights.mehendi'), color: 'text-orange-400' },
    ]
                        ...
                        >
                            ...
                            >
                                ...
                                <span className="text-primary font-bold text-xs uppercase tracking-[0.2em]">
                                    {t('booking.instantReservations')}
                                </span>
                            </motion.div>

                            <h2 className="text-4xl md:text-5xl lg:text-6xl font-heading font-bold text-gray-900 mb-6 leading-tight">
                                {t('booking.experienceBeauty')}
                                <span className="block mt-2 gold-text-animated italic">{t('booking.atYourConvenience')}</span>
                            </h2>
                            ...
                            <div className="pt-6 border-t border-gray-100 hidden lg:flex items-center gap-6">
                                ...
                                <div>
                                    <p className="text-sm font-bold text-gray-900">{t('booking.trustedBy')} {BUSINESS_STATS.happyCustomers}+ {t('booking.clients')}</p>
                                    <p className="text-xs text-gray-500">⭐ {BUSINESS_STATS.averageRating} {t('booking.averageRating')}</p>
                                </div>
                            </div>
                        </motion.div>
                        ...
                        >
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="font-bold text-gray-900">{t('booking.quickResponse')}</p>
                                        <p className="text-sm text-gray-500">{t('booking.confirmationTime')}</p>
                                    </div>
                                </div>
                            </motion.div>
                            ...
                            >
                                ...
                                <span className="text-white font-bold text-sm">{t('booking.topRated')}</span>
                            </motion.div>
                        </motion.div>
                    </div>
                </div>
                    ...
                    >
                        <div className="inline-flex items-center gap-2 px-5 py-2 bg-pink-100 rounded-full mb-8 border border-pink-200">
                            ...
                            <span className="text-pink-700 font-bold text-xs uppercase tracking-[0.2em]">
                                {t('bridal.bridalServices')}
                            </span>
                        </div>

                        <h2 className="text-4xl md:text-6xl lg:text-7xl font-heading font-bold mb-6 leading-tight">
                            {t('bridal.gettingMarried')} <span className="gold-text-animated">💒</span>
                        </h2>

                        <p className="text-xl md:text-2xl mb-12 max-w-2xl mx-auto text-gray-600 font-medium">
                            {t('bridal.description')}
                        </p>

                        {/* Bridal Features */}
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12 max-w-3xl mx-auto">
                            {[
                                { icon: Crown, text: t('bridal.features.bridalMakeup'), color: 'from-pink-500 to-rose-600' },
                                { icon: Sparkles, text: t('bridal.features.preBridalCare'), color: 'from-purple-500 to-violet-600' },
                                { icon: Users, text: t('bridal.features.familyPackage'), color: 'from-blue-500 to-cyan-600' },
                                { icon: Gift, text: t('bridal.features.specialOffers'), color: 'from-amber-500 to-orange-600' },
                            ].map((item, index) => (
                                <motion.div
                                    key={index}
                                    initial={{ opacity: 0, y: 20 }}
                    ...
                    <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                        ...
                        >
                            <div className="relative rounded-[2.5rem] overflow-hidden aspect-[4/3] shadow-2xl border-4 border-white">
                                ...
                                <div className="absolute bottom-6 left-6 right-6">
                                    <div className="flex items-center gap-3 bg-white/95 backdrop-blur-sm p-4 rounded-xl shadow-xl">
                                        ...
                                        <div>
                                            <p className="font-bold text-gray-900 text-sm">{t('training.isoCertified')}</p>
                                            <p className="text-xs text-gray-500">{t('training.professionalCourses')}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-bold text-sm">{t('training.fullPlacement')}</span>
                                </div>
                            </motion.div>
                        </motion.div>
                        ...
                        >
                            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-primary/10 rounded-full mb-6 border border-primary/20">
                                ...
                                <span className="text-primary font-bold text-xs uppercase tracking-[0.2em]">
                                    {t('training.beautyTraining')}
                                </span>
                            </div>

                            <h2 className="text-3xl md:text-5xl font-heading font-bold text-gray-900 mb-6 leading-tight">
                                {t('training.startCareer')} <span className="gold-text-animated">{t('training.beautyIndustry')}</span>
                            </h2>
                            ...
                            <div className="space-y-4 mb-10">
                                {[
                                    { text: t('training.benefits.placement'), color: 'text-green-600' },
                                    { text: t('training.benefits.practical'), color: 'text-blue-600' },
                                    { text: t('training.benefits.certification'), color: 'text-purple-600' },
                                    { text: t('training.benefits.flexible'), color: 'text-amber-600' },
                                ].map((item, index) => (
                                    <motion.div
                                        key={index}
